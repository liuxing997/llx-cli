...
const createQuestions = [
  {
    ...
    choices: [
      ...
      {
        key: ViewDesign_PROJECT,
        name: "Vue3 ViewDesign项目（vite4 + vue3 + typescript + vue router + pinia + View Design）",
        value: ViewDesign_PROJECT,
      },
    ],
  },
];
...
const create = (projectName) => {
  ...
  inquirer.prompt(createQuestions).then(({ projectType }) => {
    ...
    } else if (projectType === ViewDesign_PROJECT) {
      const createViewDesign = require("./create-viewDesign");
      createViewDesign(projectName);
    }
  });
};
